    ...
    stage('Create new build') {
      ...
      steps {
        script {
          openshift.withCluster() {
            checkout scm
            openshift.newBuild("--docker-image=registry.access.redhat.com/devtools/go-toolset-7-rhel7:latest", ".", "--name=leapi-${env.JOB_BASE_NAME}-${env.BUILD_NUMBER}-${env.CHANGE_ID}")
          }                   
        }
      }
    }
